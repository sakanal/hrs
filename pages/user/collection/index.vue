          ...
          <div v-for="publishInfo in publishInfoList">
            <el-row :gutter="20" class="my-house-info">
              ...
              <el-col :span="4" style="font-size: 16px;color: red">
                ...
                <div>
                  <el-button type="danger" size="small" @click="removeCollection(publishInfo.baseInfoId)">取消收藏</el-button>
                </div>
              </el-col>
            </el-row>
...
export default {
  ...
  // 方法集合
  methods: {
    getMyCollectionList (current = 1) {
      this.$axios.get(`/user/usercollection/myCollection/${current}`).then(response => {
    ...
    currentChange (currentPage) {
      this.getMyCollectionList(currentPage)
    },
    removeCollection(houseBaseId){
      this.$axios.delete(`/user/usercollection/removeCollection/${houseBaseId}`).then(response=>{
