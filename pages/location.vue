<template>
  <div class=''>
    ...
    <el-row type="flex" class="row-bg" justify="center">
      <el-col :span="19">
        <el-row :gutter="20">
          <el-col style="margin: 20px 0 20px 0">
            <div style="margin: 10px;font-size: 18px"><b>城市选择</b></div>
            <el-select v-model="superCityId" placeholder="省份" @change="changeCity" filterable>
              <el-option
            ...
            <el-select v-model="cityId" placeholder="城市" @change="toInfo" filterable>
              <el-option
        ...
        <el-row :gutter="20">
          <el-col style="margin: 20px 0 40px 0">
            <div style="margin: 10px;font-size: 18px"><b>按省份首字母选择</b></div>
...
export default {
  ...
  async asyncData ({ $axios }) {
    ...
    let [data, childrenList] = await Promise.all([
      await $axios.get(`/house/housecity/childrenListWithPinyin`),
      await $axios.post(`/house/housecity/childrenList`)
    ])
    // console.log(childrenList.data)
    return {
      cityList: data.data,
      selectCityList: childrenList.data
    }
  },
  data () {
    // 这里存放数据
  ...
  methods: {
    changeCity(value){
      ...
      for (let i = 0; i < this.selectCityList.length; i++) {
        if (this.selectCityList[i].id===value){
          this.childrenList=this.selectCityList[i].childrenList
          break
        }
      }
    },
    toInfo (cityId) {
      cookie.set('cityId', cityId, { domain: 'localhost' })
      this.$axios.get(`/house/housecity/info/${cityId}`)
        .then(response => {
          cookie.set('cityName', response.houseCity.name, { domain: 'localhost' })
