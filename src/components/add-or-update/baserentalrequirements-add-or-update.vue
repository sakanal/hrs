    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/house/baserentalrequirements/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.name = data.baseRentalRequirements.name
                this.dataForm.showState = data.baseRentalRequirements.showState
                this.dataForm.isDeleted = data.baseRentalRequirements.isDeleted
                this.dataForm.createdTime = data.baseRentalRequirements.createdTime
                this.dataForm.modifyTime = data.baseRentalRequirements.modifyTime
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/house/baserentalrequirements/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
